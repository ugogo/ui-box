...
test('supports inherit', t => {
  t.is(getClassName({ className: 'w' }, 'inherit'), 'ui_box-w_inherit')
})

test('supports initial', t => {
  t.is(getClassName({ className: 'w' }, 'initial'), 'ui_box-w_initial')
})

test('supports unset', t => {
  t.is(getClassName({ className: 'w' }, 'unset'), 'ui_box-w_unset')
})

test('safeValue does not transform value', t => {
  const result = getClassName({ className: 'w', safeValue: true }, '50.5%')
  t.is(result, 'ui_box-w_50.5%')
})
